...
bool Tokenizer::skipComment() {
    ...
    while (isInRange() && isCommentChar()) {
        ...
        if (*(current+1) == '[') {
            std::string::iterator close = std::findFirst("]#");
            if (!isInRange(close)) {
                eprint(
...
void Tokenizer::jumpToEnd() {
    current = sourceEnd();
}

void Tokenizer::jumpTo(const size_t& idx) {
    current = sourceBegin() + idx;
}
...
std::string::iterator Tokenizer::sourceBegin() const {
    return source->code.begin();
}

std::string::iterator Tokenizer::sourceEnd() const {
    return source->code.end();
}

std::string::iterator Tokenizer::sourceLast() const {
    return source->code.end()-1;
}

size_t Tokenizer::leftToEnd() const {
    return std::distance(current, sourceEnd())-1;
}

std::string::iterator Tokenizer::std::findFirst(const std::string& value) const {
    size_t current_idx = getIdx();
    size_t found = source->code.std::find(value, current_idx);
...
bool Tokenizer::isCommentChar() const {
    return isCommentChar(current);
}

bool Tokenizer::isCommentChar(const std::string::iterator& iterator) const {
    return isCommentChar(*iterator);
}

bool Tokenizer::isCommentChar(const char& chr) const {
    return chr == '#';
}

bool Tokenizer::isLastChar() const {
    return isLastChar(current);
}

bool Tokenizer::isLastChar(const std::string::iterator& iterator) const {
    return std::distance(iterator, sourceEnd() - 1) == 0;
}

bool Tokenizer::isInRange() const {
    return isInRange(current);
}

bool Tokenizer::isInRange(const std::string::iterator& iterator) const {
    return std::distance(iterator, sourceEnd() - 1) >= 0;
}

bool Tokenizer::isChar(const char& character) {
...
Token Tokenizer::getStringLiteral() {
    if (!isInRange())
        eprint(OutOfSourceRangeError, *source);
    ...
    if (!closed)
        eprint(UnclosedStringError, curr_token.position, *source);
...
Token Tokenizer::getNumLiteral() {
    ...
    if (isChar('0')) {
        ...
        /*
         * 'x' and 'X' are always chars... what is going on..????
         */
    ...
    Token got_token = null_token;
    while (got_token.isNothing()) {
        if (!isInRange())
            return token_create(
        ...
        switch (current_num_literal) {
            case DEC:
                got_token = pushDecDigit();
                break;
            case DEC_FLOAT:
                got_token = pushFloatDecDigit();
                break;
            case OCT:
                got_token = pushOctDigit(&non_decimal_digits, 8);
                break;
            case HEX:
                got_token = pushHexDigit(&non_decimal_digits, 8);
                break;
            default: break;
        }
    }
...
enizer::pushOctDigit(size_t* digits_counter, const size_t& max_digits) {
...
Token Tokenizer::getWordToken() {
    ...
    auto found = static_word_token_types.std::find(curr_token.str);
    return token_create (
...
Token Tokenizer::getSymbolToken() {
    ...
    auto found = search_for.std::find(*current);
...
size_t Tokenizer::getIdx() const {
    return getIdx(current);
}

size_t Tokenizer::getIdx(const std::string::iterator& iterator) const {
    return std::distance(sourceBegin(), iterator);
}
